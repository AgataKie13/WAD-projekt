...
-   liczba obserwacji: `r nrow(hotel)`
-   liczba zmiennych: `r ncol(hotel)`
...
```{r histogramy ilościowe log, echo=FALSE}
...
zróżnicowanie pozostałych obserwacji. Rozkłady zmiennych *liczba
dzieci*, *liczba niemowląt* oraz *wymagane miejsca parkingowe*
...
zmienne *wcześniejsze rezerwacje nieodwołane*, *liczba zmian rezerwacji*
oraz *liczba dni na liście oczekujących* wykazują mniejsze
zróżnicowanie - odchylenia od wartości centralnych są ograniczone, a
rozkłady pozostają bardziej zwarte. Zmienna *wcześniejsze odwołania*
...
for (i in seq(1, length(num_vars), by = 2)) {
  for (j in 0:1) {
    ...
    if (idx <= length(num_vars)) {
      ...
      qqnorm(x, pch = 1, frame = FALSE, cex = 0.6, main = wrap_title(paste("QQ plot:", var_name)))
...
for (var in zmienne_ilosciowe) {
  ...
  plot(fit, which = 4,  cex = 0.6, main = paste("Odwołanie rezerwacji ~\n", var))
}
```
...
pozostałych przypadkach, takich jak liczba specjalnych próśb czy czas
wyprzedzenia rezerwacji, wartości odległości Cooka są rzędu $10^{-3}$
...
::: {style="text-align: justify;"}
Wyjaśnij, co zrobiono z obserwacjami odstającymi: • usunięcie, •
wyrównanie wartości odstających do określonych progów, • pozostawienie
ze względu na istotność dla problemu

!na razie kod z czatu - trzeba przejrzeć zajęcia czy czegoś nie było!
:::

```{r Przekształacanie wartości odstających, include=FALSE}
# Usunięcie obserwacji wpływowych (na podstawie statystyki Cooka - obserwacje przekraczające próg 4/n)
cook <- cooks.distance(fit)
n <- nrow(hotel_noNA)
prog <- 4 / n

hotel_clean <- hotel_noNA[cook < prog, ]


# Winsoryzacja wartości odstających w zmiennych o bardzo silnej skośności
...
## wektor nazw zmiennych o dużej skośności
skewed_vars <- num_vars[abs(skew_values) > 1]

## dopasowanie nazw zmiennych do zbioru hotel_noNA
skewed_vars <- skewed_vars[skewed_vars %in% names(hotel_noNA)]

## winsoryzacja
hotel_clean <- hotel_clean %>%
  mutate(across(all_of(skewed_vars), winsorize))
```

```{r Sprawdzenie efektu przed i po usunięciu wartości odstających, echo=FALSE}
...
Usunięto `r removed` obserwacji (`r round(removed/n_before*100, 2)`%) o najwyższym wpływie (wg odległości Cooka).

```{r Sprawdzenie efektu po winsoryzacji, echo=FALSE}
...
for (var in skewed_vars) {
  ...
  # Boxplot Przed
  p1 <- ggplot(data.frame(Wartosc = x), aes(x = "", y = Wartosc)) +
  ...
  # Boxplot Po
  p2 <- ggplot(data.frame(Wartosc = x_w), aes(x = "", y = Wartosc)) +
...
hotel_std <- as.data.frame(scale(hotel_noNA))
```
