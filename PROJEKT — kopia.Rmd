...
```
::: {style="text-align: justify;"}
W zbiorze pznajdują się dwie zmienne binarne:mzmienna objaśniana - *odwołanie_rezerwacji* oraz zmienna objaśniająca *powracający_gość*.
::::
...
*Tylko dla zmiennych ilościowych*
...
for (var in zmienne_il) {
  x <- hotel[[var]]
  x_max <- quantile(x, 1, na.rm = TRUE)
  p <- ggplot(hotel, aes_string(x = var)) +
    geom_histogram(binwidth = 1, fill = "#440154", color = "white") +
    coord_cartesian(xlim = c(min(x, na.rm = TRUE), x_max)) +
    labs(
      title = paste("Histogram zmiennej:", var),
      x = "Wartość",
      y = "Częstość"
    ) +
...
::: {style="text-align:justify;"}
KOMENTARZ - Krótko skomentuj kształt rozkładu (skośność,
obecność ogonów itp.) 

Wśród zobrazowanych zmiennych warte uwagi są następujące rozkłady:
-   *liczba_specjalnych_prośb* - pokazuje, że większość gości jest zadowolona z tego, co otrzymuje,
-  *liczba_nocy 
........................
:::
