...
for (var in zmienne_ilosciowe) {
  ...
  plot(fit, which = 4, main = paste("Odwołanie rezerwacji ~\n", var))
}
```
::: {style="text-align: justify;"}
...
Opisz, jak postępowałeś z brakami danych: • usuwanie obserwacji, •
uzupełnianie braków danych Uzasadnij wybrane podejście.

```{r obsługa NA, message=FALSE, warning=FALSE}
hotel_miss <- hotel %>% select(liczba_dzieci, firma, agent_rezerwacji)
gg_miss_upset(hotel_miss)
```

```{r test MCAR, echo=FALSE}
test_result <- mcar_test(hotel_miss)
