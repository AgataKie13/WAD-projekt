...
ilosciowe_tbl %>%
  datatable(caption = "Zmienne ilościowe", rownames = FALSE, options = list(paging = FALSE))
...
można uznać, że warto wykonać analizę głównych składowych dla analizowanych zmiennych, gdyż wynoszą one w większości wiecej lub blisko 0,5. Potwierdza to również *Overall MSA* wynoszące 0,58.
:::
...
```{r wykres objaśniania zmienności przez wymiary}
...
*Nie użyto również głównych składowych wytypowanych przez analizę PCA, które zredukowały wymiar zbioru ze względu na fakt, że objaśniła ona mniej niż 70% wariancji zmienności danych.*
...
```{r statystyka GAP, message=FALSE, warning=FALSE}
library(cluster)
...
- s*tatystyka GAP* na próbce 10 tys. obserwacji, 5 inicjacjach oraz 10 symulacjach danych referencyjnych wskazuje **1 klaster** jako optymalny do użycia w dalszej analizie,
...
::: {style="text-align: justify;"}
Po przeprowadzeniu klasteryzacji metodą *k-średnich* dla 4 skupień uzyskano łączną sumę kwadratów wewnątrz klastrów *(tot.withinss)* równą `r hotel_k4$tot.withinss`, sumę kwadratów między klastrami *(betweenss)* równą `r hotel_k4$betweenss` oraz całkowitą sumę kwadratów *(totss)* na poziomie `r hotel_k4$totss`. Oznacza to, że około `r round(hotel_k4$betweenss/hotel_k4$totss, 2)`% całkowitej wariancji danych zostało wyjaśnione przez różnice między klastrami, natomiast pozostała część wynika ze zróżnicowania wewnątrz skupień.
:::
