---
title: "Wielowymiarowa Analiza Rezerwacji Hotelowych"
...
-   **reservation_status**, **reservation_status_date** → zmienne
    ...
    objaśniającą; zostały wykluczone z modelowania, aby uniknąć wycieku informacji (data
    leakage);
...
::: {style="text-align: justify;"}
W zbiorze pznajdują się dwie zmienne binarne: zmienna objaśniana -
...
ilosciowe_tbl %>%
  datatable(caption = "Zmienne ilościowe", rownames = FALSE, options = list(paging = FALSE, columnDefs = list(list(className = 'dt-center', targets = 1:7))))

binarne_tbl %>%
  datatable(caption = "Zmienne binarne", rownames = FALSE, options = list(paging = FALSE, columnDefs = list(list(className = 'dt-center', targets = 1:5))))
...
**Zmienne ilościowe** opisano za pomocą następujących statystyk: 
wartość minimalna i maksymalna, średnia, odchylenie standardowe oraz
kwantyle 1, 2 (mediana) i 3. Niepokojąca jest ujemna wartość 
*średniej ceny za noc*, którą poddano dodatkowej analizie w kolejnych 
...
których zmienna przyjmuje wartość 1. Rezerwację odwołało niecałe 38% gości, a zaledwie 3,19% odwiedzających powraca do tego hotelu.
...
```{r histogramy ilościowe log, echo=FALSE, message=FALSE, warning=FALSE}
...
for (var in logarytm) {
  ...
  p <- ggplot(hotel, aes(x = .data[[var]])) +
    geom_histogram(aes(y = after_stat(count) + 1),  # dodajemy 1, żeby log(0) nie było
...
::: {style="text-align: justify;"}
Histogramy **zmiennych ilościowych** zaprezentowano z wykorzystaniem
logarytmicznej osi OY, co pozwoliło odsłonić strukturę rozkładów
niewidoczną przy klasycznej skali. Wynika to z faktu, że we wszystkich
przypadkach zdecydowanie dominuje wartość „0”, skutecznie maskująca
zróżnicowanie pozostałych obserwacji. 

Rozkłady zmiennych *liczba_dzieci*, *liczba_niemowląt* oraz *wymagane_miejsca_parkingowe*
charakteryzują się wyraźnym skupieniem obserwacji w dolnym zakresie oraz
obecnością wartości skrajnych (np. 10 dzieci w jednej obserwacji), które
zostaną odpowiednio przekształcone w dalszej części analizy. Z kolei
zmienne *wcześniejsze_rezerwacje nieodwołane*, *liczba_zmian_rezerwacji*
oraz *liczba_dni_na_liście_oczekujących* wykazują mniejsze
zróżnicowanie - odchylenia od wartości centralnych są ograniczone, a
rozkłady pozostają bardziej zwarte. Zmienna *wcześniejsze_odwołania*
charakteryzuje się nieregularnym, „poszarpanym” przebiegiem, co utrudnia
jednoznaczną identyfikację jej rozkładu. Mimo to, podobnie jak w
przypadku pozostałych analizowanych zmiennych, największe zagęszczenie
obserwacji występuje w dolnym zakresie wartości. Wspólną cechą
wszystkich analizowanych zmiennych jest asymetria prawostronna, co
wskazuje na obecność nielicznych, lecz istotnych obserwacji o wyższych
wartościach.
:::

### 2.2.2. Histogramy dla pozostałych zmiennych ilościowych

```{r histogramy ilościowe, echo=FALSE}
...
::: {style="text-align: justify;"}
Analizowane pięć **zmiennych liczbowych** wykazuje wyraźnie zróżnicowane
kształty rozkładów, co widać na przygotowanych histogramach. W każdej z
nich obserwacje koncentrują się głównie przy niskich wartościach, jednak
tylko zmienna *czas_wyprzedzenia_rezerwacji* prezentuje typowy rozkład
prawoskośny, wskazując, że większość rezerwacji dokonywana jest z
niewielkim wyprzedzeniem, przy jednoczesnym istnieniu nielicznych,
bardzo odległych terminów. W przypadku zmiennej
*liczba_nocy_tygodniowych* rozkład jest zbliżony do normalnego, co
sugeruje równomierne rozłożenie długości pobytów tygodniowych wokół
wartości średniej. Pozostałe trzy zmienne - *liczba_specjalnych_próśb*,
*liczba_nocy_weekendowych* oraz *liczba_dorosłych* - nie wykazują
jednoznacznych, charakterystycznych form rozkładu. Histogramy pokazują
ich nieregularne, niejednorodne rozmieszczenie, z niewielkimi
skupieniami w dolnym zakresie i obecnością wartości odstających. Analiza
wizualna tych wykresów pozwala zidentyfikować zarówno typowe zachowania,
jak i obserwacje nietypowe, które mogą wymagać uwzględnienia przy
...
::: {style="text-align:justify;"}
Wykresy rozkładów **zmiennych kategorycznych** pozwoliły przyjrzeć się
strukturze danych, wskazać dominujące kategorie oraz wstępnie ocenić
zmienne potencjalnie wpływające na *odwołanie_rezerwacji.* Na podstawie
wizualizacji można zauważyć, że największe zróżnicowanie udziału odwołań
występuje w przypadku zmiennej *rodzaj_depozytu*, co sugeruje jej
kluczową rolę w dalszej analizie. Wyraźne różnice między kategoriami
widać również w zmiennych *segment_rynku*, *kanał_dystrybucji* oraz
*typ_klienta*, co odzwierciedla różne profile klientów i sposoby
dokonywania rezerwacji. Zmienne takie jak *rodzaj_wyżywienia* i
*typ_hotelu* również wykazują dominację niektórych kategorii; choć same
w sobie nie decydują bezpośrednio o odwołaniach, mogą wspomagać analizę
w połączeniu z innymi cechami, np. ceną czy długością pobytu. Wykresy
zmiennych czasowych, *miesiąc_przyjazdu* i *numer_tygodnia_przyjazdu*,
pokazują okresy planowanych pobytów i pozwalają uchwycić sezonowe wzorce
rezerwacji, które mogą pośrednio wpływać na częstość anulowań. Na tej
podstawie zmienne *rodzaj_depozytu*, *segment_rynku*,
*kanał_dystrybucji*, *typ_klienta*, a także *miesiąc_przyjazdu* i
*numer_tygodnia_przyjazdu* wydają się najbardziej wartościowe do dalszej
analizy.
:::

::: {style="text-align:justify;"}
Analiza wykresów rozkładów zmiennych liczbowych i kategorycznych
pozwoliła poznać strukturę danych i zidentyfikować obserwacje odstające.
W zmiennych liczbowych dominują niskie wartości, a niektóre, jak
*czas_wyprzedzenia_rezerwacji* czy *liczba_nocy_tygodniowych*, wykazują
wyraźne kształty rozkładu. Wśród zmiennych kategorycznych największe
różnice w udziale odwołań występują w *rodzaj_depozytu*,
*segment_rynku*, *kanał_dystrybucji* i *typ_klienta*, natomiast
*rodzaj_wyżywienia*, *typ_hotelu* oraz zmienne czasowe mogą pełnić rolę
wspomagającą. Wnioski z wykresów wskazują, które cechy warto uwzględnić
w dalszej analizie *odwołania_rezerwacji*.
:::
...
1. Test istotności korelacji zmiennych *liczba_nocy_weekendowych* oraz *liczba_nocy_tygodniowych*:

```{r testy istotności korelacji 1, include=FALSE}
...
::: {style="text-align: justify;"}
Współczynnik korelacji r = `r round(cor_test_results2$estimate, 3)` wskazuje na umiarkowaną dodatnią zależność między zmiennymi. Bardzo niska wartość p-value (≈ 0) pozwala odrzucić hipotezę zerową, co oznacza, że dłuższe pobyty obejmujące weekend częściej wiążą się również z większą liczbą nocy w dni robocze.
:::

2. Test istotności korelacji zmiennych *liczba_specjalnych_próśb* oraz *odwołanie_rezerwacji*:
```{r testy istotności korelacji 2, include=FALSE}
...
::: {style="text-align: justify;"}
Ujemna korelacja (r = `r round(cor_test_results2$estimate, 3)`) oznacza słabą, lecz istotną zależność – im więcej specjalnych próśb, tym mniejsze prawdopodobieństwo odwołania rezerwacji. Wartość p-value ≈ 0 potwierdza istotność statystyczną tej relacji.
:::

3. Test istotności korelacji zmiennych *wymagane_miejsca_parkingowe* oraz *odwołanie_rezerwacji*:
```{r testy istotności korelacji 3, include=FALSE}
...
::: {style="text-align: justify;"}
Wartość r = `r round(cor_test_results2$estimate, 3)` wskazuje na słabą ujemną korelację. Istotne statystycznie p-value sugeruje, że klienci wymagający miejsc parkingowych rzadziej odwołują rezerwacje.
:::

4. Test istotności korelacji zmiennych *czas_wyprzedzenia_rezerwacji* oraz *odwołanie_rezerwacji*:
```{r testy istotności korelacji 4, include=FALSE}
...
::: {style="text-align: justify;"}
Dodatnia korelacja r = `r round(cor_test_results2$estimate, 3)` oznacza słabą do umiarkowanej zależność – im wcześniejsza rezerwacja, tym większe prawdopodobieństwo jej odwołania. Niska wartość p-value potwierdza istotność statystyczną wyniku.
:::

5. Test istotności korelacji zmiennych *wcześniejsze_odwołania* oraz *odwołanie_rezerwacji*:
```{r testy istotności korelacji 5, include=FALSE}
...
::: {style="text-align: justify;"}
Współczynnik r = `r round(cor_test_results2$estimate, 3)` wskazuje na bardzo słabą, lecz dodatnią zależność. Pomimo niskiej siły korelacji, wynik jest istotny statystycznie, co sugeruje, że klienci z historią odwołań nieznacznie częściej anulują kolejne rezerwacje.
:::

6. Test istotności korelacji zmiennych *liczba_zmian_rezerwacji* oraz *odwołanie_rezerwacji*:
```{r testy istotności korelacji 6, include=FALSE}
...
::: {style="text-align: justify;"}
Ujemna korelacja r = `r round(cor_test_results2$estimate, 3)` wskazuje na słabą zależność – większa liczba zmian rezerwacji wiąże się z mniejszym prawdopodobieństwem jej odwołania. Istotność statystyczna potwierdzona niskim p-value.
:::

7. Test istotności korelacji zmiennych *powracający_gość* oraz *odwołanie_rezerwacji*:
```{r testy istotności korelacji 7, include=FALSE}
...
::: {style="text-align: justify;"}
Bardzo słaba ujemna korelacja (r = `r round(cor_test_results2$estimate, 3)`) wskazuje, że powracający goście nieco rzadziej odwołują rezerwacje. Mimo niewielkiej siły zależności, wynik jest istotny statystycznie.
:::
...
datatable(
  ...
  caption = "Procent braków danych (NA) dla wskazanych zmiennych") %>%
...
W dalszej części pracy zostanie omówione podejście do ich uzupełnienia
oraz odpowiednia „obróbka” braków danych, uwzględniająca charakter
poszczególnych zmiennych i wpływ na analizę.
:::

## 2.5. Analiza obserwacji odstających

::: {style="text-align: justify;"}
Analiza obserwacji odstających została przeprowadzona dla zmiennych
ilościowych.
:::
...
hotel %>%
  ...
  pivot_longer(
    ...
    names_to = "zmienna",
    values_to = "wartość") %>%
  ggplot(aes(x = zmienna, y = wartość)) +
  geom_boxplot() +
  coord_flip() +
  labs(title = "Analiza odchyleń (standaryzacja Z-score)")
```
...
num_vars <- names(hotel[zmienne_ilosciowe])
...
for (i in seq(1, length(num_vars), by = 2)) {
  for (j in 0:1) {
    ...
    if (idx <= length(num_vars)) {
      x <- na.omit(hotel[zmienne_ilosciowe][[idx]])
...
```{r Interpretacja wykresów kwantyl-kwantyl, echo=FALSE}
interpretacja <- c(
  "Skrajnie dodatnio skośny rozkład z bardzo ciężkim ogonem, wskazujący na pojedyncze wartości ekstremalne.",
  "Lekko dodatnio skośny rozkład, z niewielkim wpływem outlierów. Rozkład umiarkowanie spiczasty.",
  "Dodatnia skośność z wyraźnie ciężkimi ogonami. Pojedyncze rezerwacje o dużej liczbie nocy wpływają na spiczastość rozkładu.",
  "Wyraźna dodatnia skośność i ciężki ogon, zmienna zawiera istotne outliery.",
  "Bardzo duża dodatnia skośność i ekstremalnie spiczasty rozkład. Skrajne wartości znacząco wpływają na średnią.",
  "Znacząca dodatnia skośność i ciężkie ogony, pojedyncze wartości odstające.",
  "Skrajnie dodatnio skośny i ekstremalnie spiczasty rozkład. Większość wartości to 0 lub 1, z pojedynczymi dużymi wartościami.",
  "Dodatnia skośność i ciężkie ogony; pojedyncze rezerwacje wymagają dużo miejsc parkingowych.",
  "Lekko dodatnio skośny rozkład, umiarkowanie spiczasty. Rozkład jest stosunkowo zbliżony do normalnego, ale z kilkoma outlierami.",
  "Skrajna dodatnia skośność i spiczystość; pojedyncze rezerwacje z wieloma odwołaniami.",
  "Silnie dodatnio skośny z wyraźnym wpływem outlierów.",
  "Bardzo dodatnia skośność i spiczastość, pojedyncze rezerwacje oczekują bardzo długo.",
  "Skrajnie dodatnia skośność i spiczastość, obecność bardzo drogich rezerwacji wypaczających średnią."
)
...
datatable(tabela_interp,
          rownames = FALSE,
          caption = "Analiza skośności i kurtozy zmiennych ilościowych",
          options = list(paging = FALSE, columnDefs = list(list(className = 'dt-center', targets = 1:2))))
```

::: {style="text-align: justify;"}
Analiza rozkładów zmiennych ilościowych przy użyciu **boxplotów**, a także
wartości **skośności** ***(skewness)*** i **kurtozy** ***(kurtosis)*** pozwoliła
zidentyfikować zmienne z odchyleniami i wartościami odstającymi. Wysokie
wartości skewness wskazują na asymetrię rozkładu, a wysokie wartości
kurtosis – na spiczastość i ciężkie ogony. W takich przypadkach lepszą
miarą tendencji centralnej niż średnia jest mediana. Szczegółowe
interpretacje dla każdej zmiennej znajdują się w powyższej tabeli.
:::

### 2.5.3. Wielowymiarowa analiza odchyleń względem zmiennej objaśnianej

::: {style="text-align: justify;"}
W kolejnym kroku przeprowadzono wielowymiarową analizę odchyleń z
wykorzystaniem relacji między zmienną objaśnianą *odwołanie rezerwacji*
a zmiennymi objaśniającymi.
:::
```{r wielowymiarowa ao, echo=FALSE}
layout(matrix(c(1, 2), nrow = TRUE, byrow = 2), heights = c(2, 1, 1))

for (var in zmienne_ilosciowe) {
  # Tworzymy formułę dynamicznie
  f <- as.formula(paste("odwołanie_rezerwacji ~", var))
  
  # Dopasowujemy model liniowy
  fit <- lm(f, data = hotel)
  
  # Wykres diagnostyczny Cooka
  plot(fit, which = 4,  cex = 0.6, cex.main = 1, cex.lab = 0.8, cex.axis = 0.8, main = paste("Odwołanie rezerwacji ~\n", var))
}
```

::: {style="text-align: justify;"}
Analiza ***odległości Cooka*** dla poszczególnych zmiennych objaśniających
wykazała zróżnicowany stopień wpływu obserwacji nietypowych na
stabilność estymowanych parametrów modelu. Szczególną uwagę należy
zwrócić na zmienne *liczba_dorosłych* oraz *średnia_cena_za_noc*, gdzie
...
wskazuje na ich krytyczny wpływ na dopasowanie modelu. Znaczące wartości
odstające o charakterze punktów wpływowych obserwuje się również dla
zmiennych *liczba_niemowląt* (obserwacja 46620, $D \approx 0.3$).
Istotne zakłócenia odnotowano także w przypadku zmiennych
*wcześniejsze_rezerwacje_nieodwołane* oraz *liczba_zmian_rezerwacji*. 
Warto także zwrócić uwagę na *liczbę_dni_na_liście_oczekujących* oraz *wcześniejsze_odwołania*, w których obserwuje się pojedyncze, lecz wysokie wartości odstające.
W pozostałych przypadkach, takich jak *liczba_specjalnych_próśb* czy *czas_wyprzedzenia_rezerwacji*, wartości odległości Cooka są rzędu $10^{-3}$ lub mniejsze, co sugeruje brak dominujących outlierów zaburzających wyniki. W celu zapewnienia wiarygodności prognoz i uniknięcia biasu wynikającego z jednostkowych anomalii, rekomenduje się usunięcie obserwacji o najwyższym wpływie z wyżej wymienionych sześciu zmiennych, a następnie ponowną kalibrację modelu na oczyszczonym zbiorze danych.
:::
...
```{r obsługa NA, echo=FALSE, message=FALSE, warning=FALSE}
...
- dla zmiennej *agent_rezerwacji* ok. 9,8 tys. braków (ok. `r round(9758/16340*100, 2)`%) występuje wspólnie z brakami w zmiennej firma, a 2 braki, jak już ustalono, wraz z brakami w pozostałych zmiennych. Natomiast blisko 6,6 tys. braków (ok. `r round(6580/16340*100, 2)`%) występuje niezależnie;

- w zmiennej *firma*, poza wskazanymi zależnościami braków, zdecydowana większość wartości NA (ok. `r round(102831/112593*100, 2)`%) występuje niezależnie.
:::
...
::: {style="text-align: justify;"}
**Test Little’a** dla braków danych wykazał istotność statystyczną (χ² =
633.62, df = 6, p \< 0.001), co pozwala odrzucić hipotezę zerową o
losowym występowaniu braków (MCAR). Oznacza to, że brakujące wartości w
danych nie występują całkowicie losowo i mogą zależeć od innych
zmiennych. W związku z tym dalsza analiza wymaga zastosowania
odpowiednich metod radzenia sobie z brakami danych, takich jak imputacja
lub modele uwzględniające ich obecność.
:::
...
Brakujące wartości zmiennej agent_rezerwacji stanowią 13,69% wartości tej zmiennej i są one w 59,72% współzależne z brakami w zmiennej firma. Wobec tego, zaleca się zastosowanie prostej imputacji tych wartości średnią lub medianą wartości tej zmiennej, bądź uzupełnianie jej poprzez wielokrotną imputację, np. przy pomocy pakietu "mice". 
Z uwagi jednak na charakter i specyfikę wartości zmiennej agent_rezerwacji, tj. ID agencji podróży, która dokonała rezerwacji, zasadnym uznano usunięcie tej zmiennej z dalszej analicy.
...
```{r Usuwanie braków, include=FALSE}
hotel_noNA <- hotel %>%
  filter(!is.na(liczba_dzieci)) %>% # Usunięcie wierszy z brakami ze zmiennej liczba_dzieci
  select(-agent_rezerwacji, -firma) # Usuwanie zmiennych agent_rezerwacji oraz firma
```

```{r Sprawdzenie liczby NA po usunięciu braków, echo=FALSE}
...
datatable(
  ...
  rownames = FALSE,
  options = list(pageLength = 25, dom = 't'),
  caption = "Sprawdzenie liczby brakujących wartości po usunięciu ich ze zbioru danych",
  class = "stripe hover") %>%
  formatStyle(
    "Liczba_NA",
    target = "row",
...
Liczba obserwacji w zbiorze po usunięciu braków w zmiennej *liczba_dzieci*: **`r nrow(hotel_noNA)`**

Liczba zmiennych w zbiorze po usunięciu zmiennej *agent_rezerwacji* oraz *firma*: **`r ncol(hotel_noNA)`**
...
```{r Usunięcie wartości skrajnych, include=FALSE}
# Usunięcie obserwacji wpływowych (wg statystyki Cooka)
...
# Sprawdzenie efektu przed i po usunięciu wartości odstających
...
W konsekwencji usunięto **`r removed`** obserwacji (**`r round(removed/n_before*100, 2)`%**) o najwyższym wpływie na dopasowanie modelu.
:::
...
```{r Winsoryzacja wartości odstających w zmiennych o silnej skośności, echo=FALSE}
...
# Wybór metody winsoryzacji
## Sprawdzenie najlepszej metody winsoryzacji na zmiennej o najwyższej skośności
x <- hotel_noNA$średnia_cena_za_noc
...
# Winsoryzacja
## Wybór zmiennych o dużej skośności (>1)
...
## Wyodrębnienie zmiennych ciągłych, o dużym zakresie
continuous_vars <- c("średnia_cena_za_noc", "liczba_dni_na_liście_oczekujących")

## Sprawdzenie, czy wszystkie continuous_vars są w skewed_vars
...
::: {style="text-align: justify;"}
Dla zmiennych o szerokim zakresie wartości i wyraźnych wartościach skrajnych (takich jak *średnia_cena_za_noc* czy *liczba_dni_na_liście_oczekujących*), które mogłyby silnie wpływać na wariancję i odległości pomiędzy obserwacjami, zastosowano winsoryzację opartą na percentylach 1% i 99%. Wybór tej metody był uzasadniony tym, że reguła IQR prowadziła do modyfikacji większego odsetka danych (ok. `r round(mean(x != x_iqr) * 100, 2)`% w przypadku zmiennej o najwyższej skośności spośród wskazanych - tj. *średnia_cena_za_noc* (skew = 10,5)), co przy tak dużym zbiorze danych mogłoby prowadzić do nadmiernego wygładzenia rozkładów.

Metoda percentylowa pozwoliła ograniczyć wpływ najbardziej skrajnych obserwacji (ok. `r round(mean(x != x_p) * 100, 2)`% danych dla zmiennej o najwyższym skewness), zachowując jednocześnie strukturę zmienności istotną dla dalszych analiz wielowymiarowych. Pozwoliło to zmniejszyć wpływ ekstremów bez utraty istotnych obserwacji oraz uniknąć nadmierną modyfikację danych, charakterystyczną dla reguły IQR w przypadku rozkładów silnie skośnych.
:::

### 3.2.3. Ograniczenie zmiennych "z góry"

```{r obcięcie górnego ogona, echo=FALSE}
# Wyodrębnienie zmiennych ilościowych, dyskretnych, o niskiej rozpiętości i wysokiej skośności
count_vars <- c("liczba_niemowląt", "liczba_zmian_rezerwacji", "wcześniejsze_rezerwacje_nieodwołane", "wcześniejsze_odwołania")

# Sprawdzenie, czy wszystkie count_vars są w skewed_vars
count_vars <- count_vars[count_vars %in% skewed_vars]

# Ograniczenie wybranych zmiennych "z góry" odcinając wartości odstające
hotel_clean <- hotel_clean %>%
  mutate(across(
    all_of(count_vars),
    ~ {
...
```{r Sprawdzanie obsługi wartości odstających 1, echo=FALSE}
# Boxploty zmiennych ilościowych
vars_changed <- c(continuous_vars, count_vars, "liczba_dorosłych")
...
for (var in vars_changed) {
  ...
  ## Boxplot "przed" obsługą outlierów
...
```{r Sprawdzanie obsługi wartości odstających 3, echo=FALSE}
# Statystyki "przed" i "po" przekształceniu zmiennych
ilosciowe_tbl %>%
  datatable(caption = "Zmienne ilościowe (przed)", rownames = FALSE, options = list(paging = FALSE))
...
data.frame(
  ...
  średnia = round(sapply(hotel_clean[zmienne_ilosciowe], function(x) mean(x, na.rm = TRUE)), 2),
  ...
  row.names = NULL) %>%
  datatable(caption = "Zmienne ilościowe (po)", rownames = FALSE, options = list(paging = FALSE)) %>%
...
```{r Sprawdzanie obsługi wartości odstających 2, echo=FALSE}
# Tabela podsumowująca - skośność "przed" i "po" usunięciu wartości odstających
...
skew_compare %>% 
  ...
  datatable(
    options = list(pageLength = 25, dom = 't'),
    caption = "Sprawdzenie parametru skośności po obsłudze elementów odstających") %>%
...
# Sprawdzenie czy pokój został zmieniony
hotel_clean$zmiana_pokoju <-
  ifelse(hotel_clean$rodzaj_zarezerwowanego_pokoju !=
         hotel_clean$rodzaj_przydzielonego_pokoju, 1, 0)

# Łączna liczba zarezerwowanych nocy przez klienta
hotel_clean$liczba_nocy <-
  hotel_clean$liczba_nocy_weekendowych +
  hotel_clean$liczba_nocy_tygodniowych

# Łączna liczba gości
hotel_clean$liczba_osób <-
  hotel_clean$liczba_dorosłych +
  hotel_clean$liczba_dzieci +
  hotel_clean$liczba_niemowląt

# Sezonowość (lepsza niż miesiąc jako factor)
#hotel_clean$sezon <- factor(ifelse(
#  hotel_clean$miesiąc_przyjazdu %in% c("June","July","August"), "wysoki",
#  ifelse(hotel_clean$miesiąc_przyjazdu %in% c("December","January","February"),
#         "niski", "średni")
#))

# Tabela statystyk nowych zmiennych
nowe_zmienne <- c("zmiana_pokoju", "liczba_nocy", "liczba_osób")
...
data.frame(
  ...
  średnia = round(sapply(hotel_clean[nowe_zmienne], function(x) mean(x, na.rm = TRUE)), 2),
...
Metoda ta została wybrana, ponieważ analiza głównych składowych i analiza skupień są wrażliwe na różnice skali zmiennych, a standaryzacja pozwala wyrównać wpływ każdej zmiennej na obliczenia odległości i wariancji, bez konieczności zakładania normalności rozkładu danych.  
Nie zdecydowano się natomiast zastosować normalizacji "min-max" (unitaryzacji) lub przekształcenia ilorazowego, gdyż mogłyby niepotrzebnie zmieniać strukturę danych (np. zniekształcać odległości między punktami ujednolicając wpływ zmiennych na odległość), którym porównywalność zapewniła standaryzacja Z-score.
:::

```{r standaryzacja z-score, include=FALSE}
# Dodanie nowej zmiennej do wektora zmeinne_ilosciowe i usunięcie pominiętych
zmienne_ilosciowe <- c(zmienne_ilosciowe, "liczba_nocy", "liczba_osób") %>%
  setdiff(c("liczba_nocy_weekendowych", "liczba_nocy_tygodniowych", "liczba_dorosłych",
  "liczba_dzieci", "liczba_niemowląt"))

# Wyznaczenie zmiennych o zerowym odchyleniu
zmienne_stale <- sapply(hotel_clean[zmienne_ilosciowe], function(x) sd(x) == 0)

# Wybór zmiennych niestałych do standaryzacji
zmienne_do_standaryzacji <- zmienne_ilosciowe[!zmienne_stale]


# STANDARYZACJA ZMIENNYCH ILOŚCIOWYCH, NIESTAŁYCH
hotel_std <- hotel_clean[zmienne_do_standaryzacji] %>% 
  scale() %>% 
  as.data.frame()


# Usunięcie pominiętych zmiennych z wektora zmienne_kategoryczne
zmienne_kategoryczne <- zmienne_kategoryczne %>%
  setdiff(c("rok_przyjazdu", "dzień_przyjazdu_w_miesiącu", "rodzaj_przydzielonego_pokoju",
  "rodzaj_zarezerwowanego_pokoju"))

# Dodanie nowej zmiennej do wektora zmeinne_binarne
zmienne_binarne <- c(zmienne_binarne, "zmiana_pokoju")

# Dołączenie zmiennych kategorycznych i binarnych do zbioru hotel_std
hotel_std <- hotel_std %>%
  bind_cols(hotel_clean %>% select(all_of(c(zmienne_kategoryczne, zmienne_binarne))))

# Dołączenie zmiennej stałej do zbioru hotel_std
hotel_std$liczba_niemowląt <- hotel_clean$liczba_niemowląt
```

:::  {style="text-align: justify;"}
Finalny zbiór po obróbce zawiera **`r nrow(hotel_std)`** obserwacji i **`r ncol(hotel_std)`** zmiennych.
:::
...
1. **Analizę założeń PCA**, opartą o ocenę korelacji między zmiennymi, test Bartletta oraz kryterium KMO,

2. **Dobór liczby składowych** z wykorzystaniem kryterium procentu skumulowanej wariancji, kryterium Keisera oraz kryterium Catella (wykresu osypiska),

3. **Interpretację wyników** – przedstawiono ładunki czynnikowe ilustrujące wkład zmiennych w poszczególne wymiary, wizualizację korelacji zmiennych ze składowymi głównymi i wykres udziału zmienności objaśnianej przez składowe (cos²).
...
Do analizy włączono **`r length(zmienne_ilosciowe)` zmiennych ilościowych**, wybranych z pierwotnego zbioru zawierającego `r ncol(hotel_std)` zmiennych. Pozostałe zmienne, ze względu na charakter jakościowy, nie zostały uwzględnione w procedurze PCA.
:::
...
```{r założenie PCA - macierz korelacji, echo=FALSE, message=FALSE, warning=FALSE}
...
:::  {style="text-align: justify;"}
*H0: Macierz korelacji jest macierzą jednostkową (zmienne nie są skorelowane) - nie ma sensu    wykonywać PCA*  
*H1: Macierz korelacji nie jest macierzą jednostkową (zmienne są skorelowane) - warto wykonywać redukcję wymiaru, analizę PCA*
:::

```{r założenia PCA - test bartletta, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r założenia PCA - KMO, echo=FALSE}
...
można uznać, że **warto** wykonać ***analizę głównych składowych** dla analizowanych zmiennych, gdyż wynoszą one w większości **wiecej** lub **blisko 0,5**.  
Potwierdza to również *Overall MSA* wynoszące **0,58**.
:::
...
```{r kryterium udziału zmienności, echo=FALSE}
get_eig(pca) %>%
  rename_with(~ c("wartość własna",  "% wariancji", "skumulowany % wariancji"), .cols = c("eigenvalue",  "variance.percent", "cumulative.variance.percent")) %>%
  datatable(get_eig(pca), caption = "Kryterium udziału zmienności", options = list(dom = 't', paging = FALSE, ordering = FALSE)) %>%
  formatRound(c("wartość własna",  "% wariancji", "skumulowany % wariancji"), 2)
```

:::  {style="text-align: justify;"}
**Skumulowana wariancja procentowa** (> 80%) wskazała, że najkorzystnieszą opcją dla wybranych zmiennych jest zredukowanie ich do **`r 4-1` wymiarów*.
:::
...
:::  {style="text-align: justify;"}
**Wykres osypiska** natomiast uwidocznił punkt przegięcia w 3 wymiarze - co za tym idzie, według *kryterium Catella* należy zastosować **`r 3-1` wymiary**.
:::
...
```{r ładunki czynnikowe, echo=FALSE}
# Heatmapa
...
- **profil cenowy i preferencje klientów** (pierwsza składowa główna)  
...
:::  {style="text-align: justify;"}
Z **wykresów korelacji zmeinnych PCA** wynika, że pierwsza główna składowa (*Dim1*) wraz z drugą (*Dim2*) łącznie wyjaśniają `r 14.4+17.1`% całkowitej wariancji danych. Uwzględnienie trzeciej składowej (*Dim3*) zwiększa dodatkowo ten odsetek do `r 14.4+17.1+10.9`%. 
...
```{r wykres objaśniania zmienności przez wymiary, echo=FALSE}
# Wyświetlenie zasobów zmienności wspólnej w celu sprawdzenia jaka część zmienności danej zmiennej była wyjaśniana przez kolejne składowe
...
cos2_1 <- fviz_cos2(pca, choice = "var", axes = 1,
                       ...
                       color = "#e27396")
cos2_1 +
  ylab("Cos2 - jakość reprezentacji") +
...
Podobnie jak w przypadku analizy PCA - do przeprowadzenia analizy skupień użyto `r length(zmienne_ilosciowe)` zmiennych ze zbioru po obróbce, liczącego `r ncol(hotel_std)` zmiennych.

W kolejnych etapach dokonano:

1.  **Określania optymalnej liczby skupień** przy pomocy metody łokcia,
    indeksu sylwetkowego oraz statystyki GAP,

2.  **Właściwej analizy k-means** na pełnym zbiorze danych przy wybranej
    liczbie klastrów oraz analizy centrów skupień, 

3.  **Wizualizacji wyników segmentacji**, zarówno w przestrzeni
    zredukowanej do dwóch wymiarów, jak i na wykresach rozrzutu dla
    wybranych zmiennych, w celu oceny jakości i interpretowalności
    uzyskanych skupień.
:::
...
```{r Próbka, include=FALSE}
# Definowanie losowej próbki ze zbioru hotel_std
#set.seed(123)  # ustawianie powtarzalności próbki do dalszych analiz
#sample_idx <- sample(1:nrow(hotel_std), 15000)  # losowa próbka 5000 wierszy
#hotel_sample <- hotel_std[sample_idx, zmienne_ilosciowe]

# Zapisanie próbki do pliku w celu zapewnienia powtarzalności wyników
#saveRDS(hotel_sample, "hotel_sample_10.rds")

# Wczytanie próbek, na podstawie których przeprowadzono analizę optymalnych liczby klastrów
...
::: {style="text-align: justify;"}
Pierwsza z trzech obiektywnych metod wyboru liczby skupień jest **metoda łokciowa**, która polega na analizie zmiany sumy kwadratów odległości wewnątrz skupień *(WSS)* w zależności od liczby klastrów.  
Optymalną liczbę skupień wybiera się w miejscu, w którym dalsze zwiększanie liczby klastrów nie powoduje już istotnego zmniejszenia się wewnętrznego zróżnicowania danych *(WSS)*.
:::

```{r metoda łokciowa, echo=FALSE}
# Wyznaczenie sumy kwadratów odchyleń wewnątrz skupień (WSS) dla kolejnych wartości liczby klastrów
# + zwiększenie inicjacji wyników w celu zwiększenia ich stabilności

## Próba 5 000 obserwacji
set.seed(123)
fviz_nbclust(hotel_sample_5, FUN = kmeans, method = "wss", nstart = 5) + labs(title = "Optymalna liczba klastrów dla próbki 5 000 obserwacji")

set.seed(123)
## Próba 10 000 obserwacji
fviz_nbclust(hotel_sample_10, FUN = kmeans, method = "wss", nstart = 5) + labs(title = "Optymalna liczba klastrów dla próbki 10 000 obserwacji")
```
...
::: {style="text-align: justify;"}
Drugą metodą jest **indeks sylwetkowy**, który mierzy jakość przypisania obserwacji do skupień, porównując podobieństwo obiektów wewnątrz klastra z ich odległością od innych klastrów.  
Optymalna liczba skupień odpowiada maksymalnej średniej wartości *indeksu sylwetki*.
:::

```{r indeks sylwetkowy, echo=FALSE}
# Obliczenie średniej wartości indeksu sylwetkowego dla różnych liczb skupień w celu oceny jakości klasteryzacji

## Próba 5 000 obserwacji
set.seed(123)
fviz_nbclust(hotel_sample_5, FUN = kmeans, method = "silhouette", nstart = 5) + labs(title = "Optymalna liczba klastrów dla próbki 5 000 obserwacji")

## Próbba 10 000 obserwacji
set.seed(123)
fviz_nbclust(hotel_sample_10, FUN = kmeans, method = "silhouette", nstart = 5) + labs(title = "Optymalna liczba klastrów dla próbki 10 000 obserwacji")
```
...
Wniosek z analizy ***Silhouette Index*** jest taki, że im większa jest liczebność próbki pobranej z oryginalnego zbioru danych *(hotel_std)*, tym mniejsza liczba klastrów jest uznawana za najbardziej efektywną i dopasowaną.
Co za tym idzie - uznano, że **2-6 (ok. 4 klastry)* są najodpowiedniejszą liczbą skupisk do analizy k-means.
:::

***1) Statystyka GAP (GAP Statistic)***

::: {style="text-align: justify;"}
Ostatnią, trzecią metodą sprawdzania najodpowiedniejszej liczby klastrów do analizy skupień jest **statystyka GAP** porównująca jakość grupowania, która została uzyskana dla danych rzeczywistych z jakością grupowania dla danych losowych o tej samej strukturze.  
Optymalna liczba skupień to taka, dla której wartość *statystyki GAP* jest największa.
:::

```{r statystyka GAP, echo=FALSE, message=FALSE, warning=FALSE}
# Porównanie jakości klasteryzacji dla danych rzeczywistych z wynikami uzyskanymi dla wygenerowanych 10 zbiorów losowych (referencyjnych) (B=10)
...
# Zapis wizualizacji do png
#png("GAP_statistic_sample10.png", width = 1000, height = 600) / alt. "GAP_statistic_sample5.png"

# Wyświtlanie statytyki GAP na wykresie 
#fviz_gap_stat(gap_stat)

# Zamknięcie pliku i zapis obrazu
#dev.off()


# Wyświetlanie zapisanych wizualizacji wykresów
knitr::include_graphics("GAP_statistic_sample5.png")
knitr::include_graphics("GAP_statistic_sample10.png")
```
...
Wnioskując - **statystyka GAP** w danym przypadku nie jest jednoznaczna we wskazaniu optymalnej liczby klastrów. Zauważyć jednak można, że podobnie jak w przypadku indeksu sylwetkowego - im liczniejsza była próbka pobrana z danych *(hotel_std)* i im większy był ustalony poziom interacji, tym statystyka wskazywała coraz mniejszą liczbę klastrów.  
Uznano więc, że liczebność grup, na podstawie *statsytyki GAP* powinna zawierać się między **2-5 klastrów (3 lub 4 skupiska)**.
...
Wobec tego:  
- *metoda WSS* **nie wskazała żadnego** najefektywniejszego skupiska obserwacji, który byłby odpowiedni do dalszej analizy,  
- *indeks sylwetkowy* wskazał dwa potencjalnie istotne klastry: **2** i **6**,  
- *statystyka GAP* określiła **1** lub **5 klastrów** jako potencjalnie optymalne miejsca grupowanie się klastrów.   
...
# Klasteryzacja metodą k-średnich z podziałem na 4 skupienia
set.seed(123) # funkcja do zachowania powtarzalności wyników k-means
hotel_k4 <- kmeans(hotel_std[zmienne_ilosciowe], centers = 4, nstart = 25)
```

::: {style="text-align: justify;"}
Po przeprowadzeniu klasteryzacji metodą **k-średnich** dla 4 skupień uzyskano łączną sumę kwadratów wewnątrz klastrów *(tot.withinss)* równą `r round(hotel_k4$tot.withinss, 4)`, sumę kwadratów między klastrami *(betweenss)* równą `r round(hotel_k4$betweenss, 4)` oraz całkowitą sumę kwadratów *(totss)* na poziomie `r round(hotel_k4$totss, 4)`. Oznacza to, że około **`r round(hotel_k4$betweenss/hotel_k4$totss, 2)`% całkowitej wariancji** danych zostało wyjaśnione przez różnice między klastrami, natomiast pozostała część wynika ze zróżnicowania wewnątrz skupień.
:::

```{r centroidy klastrów, echo=FALSE}
# Wyznaczanie macierzy centrów klastrów z obiektu k-means i zmaiana jej na ramkę danych
k_centers <- as.data.frame(hotel_k4$centers)

# Dodanie numeru klastra jako osobnej kolumny
clusters <- 1:nrow(k_centers)

# Heatmapa centroidów klastrów (wartości w skali standaryzowanej)
k_centers %>%
  mutate(Cluster = factor(clusters)) %>%
  pivot_longer(cols = -Cluster, names_to = "variable", values_to = "value") %>%
  ggplot(aes(x = variable, y = factor(Cluster), fill = value)) +
    geom_tile(color = "white") +
    geom_text(aes(label = round(value, 2)), size = 3) +
    scale_fill_gradient2(low = "#4361ee", mid = "white", high = "#d81159", midpoint = 0) +
    labs(x = "Zmienne", y = "Klastry", fill = "Wartość") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

# Wyświetlanie zapisanych wizualizacji
#knitr::include_graphics("Heatmapa_centroidów.png")
```

::: {style="text-align: justify;"}
**Średnie wartości zmiennych** dla każdego klastra pokazują, jak poszczególne zmienne różnią się między grupami.  
Mapa ciepła pozowliła w prosty sposób zobczać zależności w klastrach, tzn. które zmienne są wyższe lub niższe w danym klastrze: bordowe pola oznaczają wartości powyżej średniej, niebieskie - poniżej średniej, a białe - wartości zbliżone do średniej.

Na podstawie tych średnich możemy opisać charakterystyki poszczególnych klastrów:

***Klaster 1.***  
Klaster ten posiada wyraźnie wysokie wartości w zmiennych: *wcześniejsze_odwołania* (2,93), *liczba_dni_na_liście_oczekujących* (1,95) oraz *czas_wprzedzenia_rezerwacji* (0,92).
można więc sądzić, że skupia on klientów, którzy planują swoje pobyty w hotelu z dużym wyprzedzeniem. Charakteryzują się oni dłuższym niż przeciętnie okresem czasu, jaki upływa od momentu dokonania rezerwacji pokoju do planowanego przyjazdu, a co się z tym wiąże - długim okresem przebywania na liście oczekujących, a także dużą liczbą wcześniejszych rezerwacji i ich odwołań. Sugeruje to, że są to osoby często rezerwujące noclegi, ale jednocześnie skłonne do zmiany planów.  
Pozostałe zmienne mają wartości średnie lub niższe od średniej, więc nie wyróżniają tej grupy.

***Klaster 2.***  
Klienci należący do tego klastra wyróżniają się wysokim zapotrzebowaniem na miejsca parkingowe *(wymagane_miejsca_parkingowe: 3,84)*. Może to wskazywać, że często podróżują oni samochodem, stąd też większe znaczenie ma dla nich dostępność parkingu przy wyborze noclegu.  
Pod względem pozostałych analizowanych zmiennych ich zachowanie jest zbliżone do przeciętnego, co sugeruje, że są to typowi goście, z tylko jednym, dodatkowym wymaganiem.

***Klaster 3.***  
Klaster trzeci obejmuje rezerwacje potencjalnie liczniejsze i o wyższym standardzie. Charakteryzowni przez niego klienci podróżują w większych grupach *(liczba_osób: 0,65)*, składają więcej specjalnych próśb *(liczba_specjalnych_próśb: 0,81)* oraz decydują się na noclegi o wyższej cenie *(średnia_cena_za_noc: 0,75)*. Może to wskazywać na rodziny, grupy zorganizowane lub gości o podwyższonych oczekiwaniach co do jakości usług.

***Klaster 4.***  
Ostatni klaster reprezentuje najbardziej przeciętnych klientów, bez wyraźnych oczekiań ani szczególnych preferencji dotyczących warunków pobytu. Wszystkie analizowane zmienne przyjmują tu wartości bliskie średniej lub nieco niższe, co oznacza brak wyraźnych cech charakterystycznych. 
Klaster ten więc odzwierciedla zachowania standardowych klientów, stanowiąc punkt odniesienia dla pozostałych grup.
:::


### 5.2.3. Wizualizacja wyników segmentacji

```{r wizualizacja wyników analizy, echo=FALSE}
# Wizualizacja wyników klasteryzacji w przestrzeni wielowymiarowej (dwa wymiary)
fviz_cluster(hotel_k4, data = hotel_std[zmienne_ilosciowe], geom = "point")

# Wyświetlanie zapisanych wizualizacji
#knitr::include_graphics("Wyniki_zanaliza_k_means.png")
```
...
**Klaster 2** ***(zielony)*** wykazuje największą rozpiętość i obejmuje obserwacje o bardziej zróżnicowanych profilach, podczas gdy **klaster 4** ***(fioletowy)*** tworzy stosunkowo zwartą grupę, co sugeruje spójność zachowań w tej grupie. **Klaster 1** ***(czerwony)*** i **klaster 3** ***(niebieski)*** również tworzą rozpoznawalne obszary, z lekkim nakładaniem się z innymi skupiskami, co może odzwierciedlać pewne wspólne cechy między grupami klientów.  
Wykres klasteryzacji zatem zachowuje wszystkie własności, które wytypowano przy omawianiu centroidów poszczególlnych z grup.
:::

```{r wykres rozrzutu dla dwóch wybranych zmiennych, echo=FALSE}
# Dołączenie informacji o przypisaniu obserwacji do klastrów
hotel_group <- data.frame(hotel_clean[zmienne_ilosciowe], Group = hotel_k4$cluster)

# Obliczenie średnich wartości zmiennych w poszczególnych klastrach 
hotel_centers <- as.data.frame(aggregate(.~Group, data = hotel_group, FUN = mean))

# Konwersja numeru grupy na zmienną kategoryczną
hotel_group$Group <- as.factor(hotel_group$Group)

# Wizualizacja klastrów na wykresie - "zmienna" vs "zmienna"
ggscatter(x = "średnia_cena_za_noc", y = "czas_wyprzedzenia_rezerwacji", color = "Group", data = hotel_group)

# Wyświetlanie zapisanych wizualizacji
#knitr::include_graphics("Wykres_punktowy_zmiennych.png")
```
...
contrib_df %>%
  pivot_longer(
    ...
    names_to = "Składowa",
    values_to = "Wkład") %>%
  ggplot(aes(x = Zmienna, y = Wkład, fill = Składowa)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~ Składowa, scales = "free_x") +
  ...
  geom_hline(yintercept = 10,
             ...
             color = "red") +
  geom_text(aes(
    label = paste0(round(Wkład, 1), "%"),
    hjust = ifelse(Wkład < 5.5, -0.1, 1.1)),
    size = 2.5) +
  coord_flip() +
  labs(title = "Wkład zmiennych w główne składowe PCA",
       y = "Wkład (%)",
       x = "") +
  theme_minimal()
```
...
```{r wykres radarowy - k-means, echo=FALSE}
# Wizualizacja centroidów zmiennych w poszczególnych klastrach
radar_data <- rbind(
  apply(k_centers, 2, max),
  apply(k_centers, 2, min),
  k_centers)

rownames(radar_data) <- c("Max", "Min", paste0("Klaster_", clusters))

colors_border <- rainbow(nrow(k_centers))

## Rysowanie wykresu radarowego
radarchart(radar_data,
           axistype = 1,
           pcol = colors_border,
           pfcol = alpha(colors_border, 0.3),
           plwd = 2,
           cglcol = "grey",
           cglty = 1,
           axislabcol = "grey",
           caxislabels = round(seq(min(radar_data[2, ]), max(radar_data[1, ]), length.out = 5), 1),
           vlcex = 0.65)
legend("topright", legend = paste0("Klaster ", clusters),
       bty = "n", pch = 20, col = colors_border, cex = 0.8)

# Wyświetlanie zapisanych wizualizacji
#knitr::include_graphics("Wykres_radar.png")
```
