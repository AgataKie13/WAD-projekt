...
Dodatkowo, przed przystąpieniem do klasteryzacji wszystkie zmienne ilościowe poddano standaryzacji, co zapewniło ich porównywalność i zapobiegło dominacji zmiennych o większej skali.
...
2.  **Właściwą analizę k-means** na pełnym zbiorze danych przy wybranej
    liczbie klastrów, z wykorzystaniem 25 losowych inicjalizacji
    centroidów, w celu zwiększenia stabilności wyników,

3.  **Analizę centrów klastrów**, co zakładało wyznaczenie średnich
    wartości zmiennych w poszczególnych skupieniach oraz ich
    interpretację,

4.  **Przypisania obserwacji do klastrów** i dołączenie informacji o
    przynależności do oryginalnego zbioru danych,

5.  **Wizualizacji wyników segmentacji**, zarówno w przestrzeni
...
::: {style="text-align: justify;"}
*Z uwagi na bardzo dużą liczebność zbioru danych, obliczenia znajdujące
się w tym kroku zostały przeprowadzone na losowej próbie obejmującej
5000 obserwacji. Próba ta była konieczna, ponieważ funkcje służące do
wyznaczania miar jakości klasteryzacji wymagają tworzenia dużych
macierzy odległości, których obliczenie dla pełnego zbioru przekraczało
możliwości obliczeniowe programu R. Dodatkowo podjęto próbę
wykorzystania pakietu NbClust, jednak nawet po ograniczeniu liczby
obserwacji funkcja ta nie zadziałała poprawnie z uwagi na zbyt dużą
złożoność obliczeń.*
::::
```{r Próbka, include=FALSE}
# Definowanie losowej próbki ze zbioru hotel_std
#set.seed(123)  # ustawianie powtarzalności próbki dla dalszych 
...
# + zwiększenie inicjacji wyników w celu zwiększenia stabilności wyników
...
::: {style="text-align: justify;"}
Ostatnią, trzecią metodą jest *statystyka GAP* porównująca jakość grupowania, która została uzyskana dla danych rzeczywistych z jakością grupowania dla danych losowych o tej samej strukturze.  
...
# kod na próbce 10 tys. obserwacji, nstart = 5, B = 10 wskazuje 1 klaster
# kod na próbce 10 tys. obserwacji, nstart = 1 (domyślnie) i B = 20 wskazuje 1 klaster
# kod na próbce 5 tys. obserwacji, nstart = 5, B = 10 wskazuje 10+ klastrów
# kod na próbce 5 tys. obserwacji, nstart = 1 (domyślnie) i wskazanych parametrach wskazuje 10+ klastrów
```

::: {style="text-align: justify;"}
Przez wzgląd na długość wyświetlania wykresu dla próbki, zdecydowano się ująć wynik słownie.

- statystyka GAP na próbce 10 tys. obserwacji, 5 inicjacjach, B = 10 wskazuje 1 klaster
- statystyka GAP na próbce 10 tys. obserwacji, nstart = 1 (domyślnie) i B = 20 wskazuje 1 klaster
- statystyka GAP na próbce 5 tys. obserwacji, nstart = 5, B = 10 wskazuje 10+ klastrów
- statystyka GAP na próbce 5 tys. obserwacji, nstart = 1 (domyślnie) i B = 10 wskazuje 10+ klastrów

WNIOSEK:
Statystyka GAP w danym przypadku nie jest jednoznaczna we wskazaniu optymalnej liczby klastrów. Zauważyć jednak można, że im liczniejsza była próbka pobrana z danych *(hotel_std)* i im większy był ustalony poziom interacji, wskazywała ona coraz mniejszą liczbę klastrów.  
Uznano więc, że wskazana liczba zna
:::

### 5.2.2. Właściwa analiza *k-means*

```{r k-means, echo=FALSE}
# Klasteryzacja metodą k-średnich z podziałem na 6 skupień
...
### 5.2.3. Analiza centrów klastrów

```{r}
# Wyświetlenie centroidów klastrów (wartości w skali standaryzowanej)
hotel_k6$centers
...
### 5.2.4. Przypisanie obserwacji do klastrów
...
# Zamiana na długi format (long) do ggplot
...
ggplot(hotel_long, aes(x = Variable, y = Mean, fill = factor(Group))) +
  ...
  labs(fill = "Klaster", x = "Zmienna", y = "Średnia") +
  theme_minimal() +
...
# fmsb wymaga, żeby dane były w formacie: pierwsze dwa wiersze = max i min dla skali
...
### 5.2.5. Wizualizacja wyników segmentacji
...
::: {style="text-align: justify;"}
Wizualizacja wyników klasteryzacji z wykorzystaniem metody k-średnich
dla sześciu klastrów wskazuje na wyraźną segmentację obserwacji.
Większość skupień tworzy spójne i rozpoznawalne grupy, choć część z nich
częściowo nakłada się na siebie, co wynika z wielowymiarowego charakteru
danych. Widoczna struktura klastrów potwierdza jednak istnienie
zróżnicowanych profili rezerwacji, a zastosowanie metody k-means można
uznać za uzasadnione i efektywne w kontekście analizowanego zbioru.
:::
...
library(ggpubr)
ggscatter(x = "liczba_nocy", y = "Liczba_osób", color = "Group", data = hotel_group)
```
...
# 6. PORÓWNANIE METOD I WIZUALIZACJA WYNIKÓW
