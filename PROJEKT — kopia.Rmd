...
hotel <- hotel %>%
  ...
  "liczba_specjalnych_prośb" = "total_of_special_requests",
...
binarne <- names(hotel)[sapply(hotel, is_binary)]

binarne
str(hotel)

str(hotel$wcześniejsze_odwołania)
str(hotel$wcześniejsze_rezerwacje_nieodwołane)
str(hotel$wymagane_miejsca_parkingowe)
str(hotel$liczba_specjalnych_prośb)
...
num_cols <- sapply(hotel, is.numeric) & !(names(hotel) %in% binarne)
vars <- names(hotel)[num_cols]

summary_tbl <- data.frame(
  zmienna    = vars,
  min        = sapply(hotel[vars], function(x) min(x, na.rm = TRUE)),
  max        = sapply(hotel[vars], function(x) max(x, na.rm = TRUE)),
  średnia    = sapply(hotel[vars], function(x) mean(x, na.rm = TRUE)),
  sd         = sapply(hotel[vars], function(x) sd(x, na.rm = TRUE)),
  Q1         = sapply(hotel[vars], function(x) quantile(x, 0.25, na.rm = TRUE)),
  mediana    = sapply(hotel[vars], function(x) median(x, na.rm = TRUE)),
  Q3         = sapply(hotel[vars], function(x) quantile(x, 0.75, na.rm = TRUE)),
  row.names  = NULL
)
...
*Tylko dla zmiennych numerycznych*
...
ggplot(hotel_long, aes(x = value, fill = zmienna)) +
  geom_histogram(
    bins = 30,
    color = "white"
  ) +
  facet_wrap(~zmienna, scales = "free", nrow = 5) +
  labs(
    x = "Wartość",
    y = "Częstość",
    title = "Rozkłady zmiennych"
  ) +
...
zmienne nie mające sensu: rok_przyjazdu, ::: {style="text-align:
justify;"} KOMENTARZ - Krótko skomentuj kształt rozkładu (skośność,
obecność ogonów itp.) :::
...
::: {style="text-align: justify;"}
Z analizy macierzy korelacji można wywnioskować, że najsilniejsze
zależności występują odpowiednio między ...
:::
